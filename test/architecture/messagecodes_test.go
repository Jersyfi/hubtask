...
var messageCode = regexp.MustCompile(`"((?:route|request|access|accounts|groups|memberships|idempotency|config|errors|dependency|capacity|containers|items|buckets|labels|comments|media|ordering|events|sync|storage|audit|usecase|shared|automation|lifecycle|activity|query)\.[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)*)"`)
