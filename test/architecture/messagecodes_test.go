...
var messageCode = regexp.MustCompile(`"((?:route|request|access|accounts|groups|memberships|idempotency|config|errors|dependency|capacity|containers|items|buckets|labels|comments|fields|media|ordering|events|sync|storage|audit|usecase|shared|automation|lifecycle|activity|query|views|notifications|email|mail|jobs|crypto|calendar)\.[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)*)"`)
