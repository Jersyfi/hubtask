...
func TestPG2AnErasureLeavesNothingBehind(t *testing.T) {
	...
	subject, tenant, email := seedErasableSubject(ctx, t)

	eraser := privacyservice.Eraser{
		...
		Clock:      portclock.Fixed(time.Now().UTC()), TombstoneWindow: 90 * 24 * time.Hour,
	}
...
// they are.
func seedErasableSubject(ctx context.Context, t *testing.T) (subject, tenant shared.ID, email string) {
	t.Helper()
	...
	statements := []struct {
		...
		{`INSERT INTO tenant (id, slug, display_name) VALUES ($1, $2, 'PG-2')`,
			[]any{tenant.String(), "pg2-" + strings.ReplaceAll(tenant.String(), "-", "")[:20]}},
		...
		{`INSERT INTO media_object (id, tenant_id, storage_key, mime_type, byte_size, checksum, usage, created_by)
		  VALUES ($1, $2, $3, 'text/plain', 12, $4, 'ATTACHMENT', $5)`,
			[]any{freshID().String(), tenant.String(), "media/" + subject.String(),
