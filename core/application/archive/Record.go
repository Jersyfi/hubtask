...
var excluded = map[string]string{ //nolint:gosec // G101: table names and prose, one of which is called access_token
	...
	"session":               "§8.4 - a restore does not make old credentials valid again; a restored session is a stolen archive's way in",
	"session_refresh_token": "the session's refresh chain, §8.4's reasoning verbatim",
