...
func TestTheDefaultTrashPeriodIsThirtyDaysWithASevenDayFloor(t *testing.T) {
	defaults := lifecycle.DefaultPolicies()
	if len(defaults) != 1 {
		t.Fatalf("%d default policies, want 1 - the trash", len(defaults))
	}

	trash := defaults[0]
	if trash.DataKind != lifecycle.KindTrash {
		t.Errorf("the default is for %s, want %s", trash.DataKind, lifecycle.KindTrash)
	}
