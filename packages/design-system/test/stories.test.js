...
test('design-system.md §4 still reads as an inventory', () => {
  ...
  // Wave 1 is the one F1 builds, and §4 names its twenty by hand - eighteen, plus the `Icon` that
  // F1-03 brought forward because `IconButton` cannot be built before there is one, plus the
  // `ProgressBar` F4-06 added when `UploadField` turned out to have hand-rolled one. If this
  // number moves, a component was added or removed in the specification and the waves need
  // re-reading.
  assert.equal(waves.get('Wave 1').length, 20, 'wave 1 is no longer the twenty §4 lists');
});
