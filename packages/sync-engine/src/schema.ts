...
/** The seven things a device can do offline, as the contract names them (offline-sync.md §3.2). */
export type MutationKind = 'ITEM_CREATE' | 'ITEM_PATCH' | 'ITEM_DELETE' | 'SET_ADD' | 'SET_REMOVE' | 'MOVE' | 'COMMENT_ADD';

/** The four sets an entry carries, as `SET_ADD` and `SET_REMOVE` name them. */
export type SetName = 'labels' | 'members' | 'watchers' | 'attachments';

/**
 * What a write becomes when it is queued: the mutation without its clocks. The application
 * answers this from a write's method, path and body (`mutationFor`, the counterpart of
 * `pathsFor`); the engine mints the operation identifier and stamps the fields, because the clock
 * and the device are the engine's.
 */
export interface QueuedWrite {
  readonly kind: MutationKind;
  /** The entry the mutation is about - the client-minted identifier on an ITEM_CREATE (§9.1). */
  readonly itemId: string;
  readonly baseVersion?: number | null;
  /** ITEM_PATCH: the fields that moved, each stamped with its own clock by the engine. */
  readonly fields?: Readonly<Record<string, unknown>>;
  /** SET_ADD and SET_REMOVE: which set, and which element. */
  readonly set?: SetName;
  readonly element?: string;
  /** ITEM_CREATE, MOVE and COMMENT_ADD: what the applier takes; a comment's `id` is minted here too. */
  readonly payload?: Readonly<Record<string, unknown>>;
  /** Which held reads the write makes stale, as the caller named them. */
 ...
 *
 * Held in the store's `queue` collection in the order it was made, which is the order it is
 * pushed in (§3.2: ordering within one device is preserved). `id` is the operation identifier -
 * minted once per intent, so a repeated push takes effect exactly once (§9.2) - and `mutation` is
 * the wire shape, stamped, as it will be sent however many times it takes.
 */
export interface PendingMutation {
  /** The operation identifier, a UUIDv7. */
  readonly id: string;
  /** The order it was made in: what the push sorts by. */
  readonly seq: number;
  readonly kind: MutationKind;
  readonly itemId: string;
...
/**
 * A mutation the server refused (§9.5): kept with its code until the person dismisses it, so
 * that what they did offline is shown to them rather than swallowed. `sync.gone` (§7) and
 * `forbidden` (§6) are the two codes the requirements name; `local` is what the mutation
 * carried, offered back for safekeeping.
 */
...
export interface ConflictRecord {
  ...
  readonly preservedCommentId?: string;
  readonly at: number;
}

/** What `engine.queue()` publishes: what `SyncStatus` renders. */
export interface QueueState {
  /** How many mutations wait to be pushed. */
  readonly count: number;
  /** When the oldest of them was made, by the injected clock. */
  readonly oldestAt?: number;
  /** Whether a push is on its way. */
