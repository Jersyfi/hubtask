...
| Backups | **35 days** (P-5), binding for the system backups and the point-in-time window the operator keeps. **Enforced by the storage since H-10**, not only by this document: the database cluster's own `retentionPolicy` is that number ([`k8s/templates/cnpg-cluster.yaml`](../../k8s/templates/cnpg-cluster.yaml)), and the backup bucket's Object Lock retention is the same number, so the archive cannot outlive the period and the period cannot outlive the lock. Where an installation keeps its system backup as a `pg_dump` instead ([backup-restore.md §8.6](./backup-restore.md#86-the-minimal-path-a-dump-and-what-it-does-not-give)), the period is the operator's own rotation and this row is the promise they are making. Deletion takes effect in the primary system at once and in the backups when that period has elapsed — a fact made transparent to the data subject rather than concealed. Two things make the number honest rather than decorative: the system backup plan is bounded by it instead of keeping monthly and yearly generations, and a point-in-time restore that rewinds past a completed erasure re-applies the erasures from the period it rewound before the system is opened to traffic ([backup-restore.md](./backup-restore.md) §7) |
...
| **Cyber Resilience Act** (Reg. 2024/2847, obligations from late 2027) | Concerns "products with digital elements" made available commercially. Provision without commercialisation is outside its scope — which is where Hubtask stands until Licensing Start ([ADR-0059](../adr/ADR-0059-licensing-phases-and-licensing-start.md)). From Licensing Start, every major that licences have been sold for is a **commercial offering** and the manufacturer obligations attach to it; version lines without sold licences stay outside | The technical provision is in place — an SBOM per release, coordinated disclosure via `SECURITY.md`, signed artefacts, documented secure-by-default settings. The *process* obligations — reporting through the ENISA platform, a disclosure policy, a declared support period per licensed major, vulnerability handling with deadlines — are the owner's prerequisites for Licensing Start ([licensing-editions.md](./licensing-editions.md) §8); until then `SECURITY.md` states aims, not deadlines |
...
| P-4 | Have legal advice assess CRA applicability to the commercial variant; set the support period per licensed major | Before Licensing Start — an owner prerequisite ([licensing-editions.md](./licensing-editions.md) §8), no longer tied to a version |
