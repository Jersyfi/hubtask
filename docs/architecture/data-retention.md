...
| `AI_SUGGESTION` | `created_at` | 30 days | What AI proposed and nobody has to keep (J-05). The shortest default in this table on purpose: a suggestion is about a *state* of an entry, and an entry's state does not stay still — one nobody answered in a month is one about a task that has moved on. Both decided states age out with the proposals, because an accepted suggestion has already become the entry's own history, which is where "why does this task say that" is answered for good; the row is the working note, not the record. No marking phase, the jumble's reasoning; a tenant-wide legal hold does stop the sweep |
