...
| AI provider | AI suggestions disappear and search stops finding entries by what they mean — two features from one dependency, so the report carries both `ai_suggestions` and `semantic_search` (J-10). Every manual route remains, and search still finds the words somebody typed. One breaker **per endpoint** (J-03), because a provider is per tenant and a single breaker would let one workspace's dead endpoint switch off everybody's; `/meta/health` reports the installation's view — `disabled` until this process has called a provider at all, then `ok` or `down` — and never names an endpoint. Proved against a real stopped container in `TestRT1AStoppedContainerDegradesExactlyItsOwnFeature` (J-04, evidence [RT-1-2026-09-09.md](../evidence/RT-1-2026-09-09.md)), and the *disabled* half against an installation configured with no provider at all (J-17, evidence [QS-09-2026-09-09.md](../evidence/QS-09-2026-09-09.md)) - where the report is `ok` with an empty `degraded_features`, because an installation that never wanted AI has lost nothing | The feature is greyed out with a reason; where there is no provider at all there is no control, which the client reads from `/meta/capabilities` rather than from a refusal |
