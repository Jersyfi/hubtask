...
* **A restore writes rows without change log entries**, so a device that was offline through one keeps a cursor that is still valid and is never told what changed. That is open point B-5 in [backup-restore.md](./backup-restore.md) §12 rather than a property to rely on: until it is settled, a workspace that has been restored has to be resynchronised on the devices by hand.
