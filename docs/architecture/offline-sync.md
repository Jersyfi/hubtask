...
| Account preferences | `locale`, `time_zone`, `week_start`, `celebrations`, `onboarding_completed_at` on the account (F6-12) | **Last writer wins by the account's own write**, through `UpdateAccountPreferences` over the API and never through a push: an account is not a synchronised entity (§1), so a device that changed a preference offline writes it when it is back and the later write stands - which is right for a switch a person flips and a moment they passed |
