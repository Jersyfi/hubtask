...
| R-08 | The absent frontend decision delays user feedback | Late insight | Answered: the framework is decided (ADR-0030) and the client is the tool the work is done in. A day's work was walked through it in [F2-16's pass](../evidence/R-08-2026-09-04.md) — the route is written down so the next milestone's walk can be compared with it, and what the application could not do is a list of named issues rather than a feeling. What remains of the risk is that the pass is done by the people who built it: R-04's early prototype is no longer the countermeasure, walking the route is |
