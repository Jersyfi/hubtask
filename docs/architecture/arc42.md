...
| R-08 | The absent frontend decision delays user feedback | Late insight | Answered, and walked twice. The framework is decided (ADR-0030) and the client is the tool the work is done in. [F2-16's pass](../evidence/R-08-2026-09-04.md) found four writes that reached no screen; [F3-20's](../evidence/R-08-2026-09-08.md) walked nineteen steps end to end and found none — all eight of the first walk's issues are closed, and every finding of the second is the client and the server describing the same thing differently. What remains of the risk has two parts: the pass is done by the people who built it, and the second walk was driven through the API rather than the interface, so the kind of defect where a control exists and behaves differently from the server is the kind neither walk has looked for |
