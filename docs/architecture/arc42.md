...
| QS-24 | Two people edit the same item offline: A changes the due date, B the title | Both changes survive (per-field merging); no user decision is needed (test SY-1). Walked at the end of `0.8.5` on the integration environment as two `hubctl` profiles: A created the entry, B changed the title and A the due date each in a queue of its own, both pushed `APPLIED`, and each device's next delta carried the other's field as its own change log entry under the reading that decided it ([SY-2026-09-16.md](../evidence/SY-2026-09-16.md)) |
| QS-25 | A device has been offline for 90 days and knows items deleted in the meantime | `sync.cursor_too_old` forces a full sync; deleted objects do not come back (tests SY-5, RE-6). Walked with a cursor minted ninety-one days back by the installation's own codec: refused in the catalogue's words, accepted at eighty-nine; the full synchronisation delivered twelve records and not the entry deleted in between ([SY-2026-09-16.md](../evidence/SY-2026-09-16.md)) |
| QS-26 | A user loses access to a collection while offline | `ACCESS_REVOKED` in the pull stream, the client deletes locally; mutations against it are rejected server-side (test SY-6). Walked: Bert's role on the hub revoked while his device was away with a queued creation; the pull delivered one `ACCESS_REVOKED` record at the hub addressed to him, the push was `REJECTED` with `forbidden`, and the owner's device saw no record ([SY-2026-09-16.md](../evidence/SY-2026-09-16.md)) |
| QS-27 | A device with a clock three hours out synchronises | The HLC bound stops it outvoting everyone else (test SY-2). Walked with `hubctl sync push --clock-offset 3h`: the device stamped a reading three hours ahead, the server recorded the change under the device with its own reading, and an edit two seconds later from a correct clock won ([SY-2026-09-16.md](../evidence/SY-2026-09-16.md)) |
...
| R-08 | The absent frontend decision delays user feedback | Late insight | Answered, and walked three times. The framework is decided (ADR-0030) and the client is the tool the work is done in. [F2-16's pass](../evidence/R-08-2026-09-04.md) found four writes that reached no screen; [F3-20's](../evidence/R-08-2026-09-08.md) walked nineteen steps through the API and found none; [F4-21's](../evidence/R-08-2026-09-11.md) walked the administration area — seventeen screens that had never been opened — in a browser, signed in with a password and a second factor, and found one missing surface (#542, a picker the manifest gives nothing to offer) and one retry the server answers before it reads (#543). The third walk also settled the second's open question: driving the interface finds what reading it cannot — three aggregates read back without their tenant, invisible to every test and every earlier walk, surfaced only when finished screens were driven against a real server. What remains of the risk is one thing: the pass is done by the people who built it |
