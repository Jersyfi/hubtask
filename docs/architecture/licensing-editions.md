...
| The TypeScript SDK | `packages/api-client/src/client.gen.ts` and `packages/api-client/examples/` | The one package that holds both licences; the file boundary is ADR-0059 Appendix B, and the first-party types beside it stay `BUSL-1.1` |
| The API contract and the event schemas | `api/openapi.yaml`, `api/openapi.json`, `api/events/` | What every SDK is generated from |
| The n8n community node | `packages/n8n-nodes-hubtask/` | [ADR-0058](../adr/ADR-0058-connector-packages.md); the Zapier app is not named and stays under the repository licence |
...
* **The SDKs, the contract and the n8n package** — Apache-2.0, in this repository (§9).
