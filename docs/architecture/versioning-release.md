...
| Data protection | The gates PG-1…PG-8 ([data-protection.md](./data-protection.md) §10): a recorded change without a classification (PG-1), a table in the archive the export has decided nothing about (PG-3), a content attribute in a log, a metric, a trace or a problem document (PG-4), a retention period outside its bounds (PG-5) and an address written into the source (PG-6) fail every pull request through `make gate-privacy`; the erasure sweeping every storage location (PG-2) and the schema reconciled against the data catalogue (PG-7) run in the nightly with containers through `make gate-privacy-full`; PG-8 is a tripwire until an AI provider surface exists (`0.7.0`). Each is proved to go red by `make gate-selftest` |
