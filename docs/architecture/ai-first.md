...
| Semantic search | Embeddings of titles/notes in `pgvector`, hybrid search with `tsvector`. **Shipped in J-10**: one ranked page with one cursor, the words winning over the meaning, and pgvector detected rather than demanded ([ADR-0050](../adr/ADR-0050-pgvector-as-a-capability.md)). **The index holds 1536 dimensions, and which models fit it was undocumented until [#551](https://github.com/Jersyfi/hubtask/issues/551) in `0.8.0`**: a narrower model is stored exactly, zero-padded — cosine is the only distance the product uses, and padding changes neither the dot product nor either norm — and a wider one is refused with `ai.embedding_too_wide` — before the first text is sent where the provider can say its width, at the first batch where it cannot — the job finishing rather than retrying, the search staying lexical, and both `/meta/capabilities` and `/meta/health` saying so ([ADR-0054](../adr/ADR-0054-embedding-width.md), [#569](https://github.com/Jersyfi/hubtask/issues/569)). Until then the two Ollama embedding models in common use, 768 and 1024 wide, failed at insert with a raw database error, and semantic search was in effect OpenAI-only with nothing saying so | Search result |
| Template generation | A natural language description → `Template`. **Shipped in P-11** (`0.9.0`), where `0.7.0` decision 10 moved it: `AiGenerateTemplate` takes a description in the caller's words and the collection the template would belong to, and asks the workspace's provider — as a job, through the budget, the consent and the breaker exactly as the fields are asked — for a template in the contract's own shape: a name, and one root node with its tree, each node a type, a title, optional notes and a relative due offset. The material tells the model the shape a template may take here — which types sit in a collection and what may sit under each, read from the same profiles `CreateTemplate` checks — and the narrowing holds the answer to it: a node the profile refuses under its parent is dropped with everything under it, counted, and the count is recorded on the suggestion (`dropped_nodes`, [#767](https://github.com/Jersyfi/hubtask/issues/767)) as well as in the job's result (`nodes_dropped`) — the suggestion, because that is what a client renders and `:generate` answers no `JobRef`; an offset `CreateTemplate` would refuse is left off its node. The words are the one material that exists in no row the workspace holds, so they are held under row level security (`ai_request`) for the job that reads them and deleted when it ends — the queue keeps carrying references only. The answer is a `TEMPLATE` suggestion with the collection as its target and a `TemplateInput` as its payload; `AcceptSuggestion` on it is `CreateTemplate` performed as the accepting person, the scope written from the target rather than read from the answer, with their rights at the collection and its own audit entry. A client's suggestion strip needs nothing new: the payload is a template | Suggestion, defined by the person who accepts it |
| Translation | Item content on request. **Shipped in M-11** (`0.8.0`), where `0.7.0` decision 10 moved it: `AiTranslate` reads one entry's title and notes in a language the caller names — or the caller's own — through the workspace's provider, and hands the answer back with its provenance. It is a *read*, in the sense the search's meaning is one: the entry's own read is the permission asked, the consent and the budget are asked after it, the call is bounded because a person is waiting, and every way of not getting an answer is `ai.unavailable`. Nothing is stored — no suggestion, no acceptance, the original stays as it is (`i18n-l10n.md` §7) — and the audit records the entry and the language, never the text. The surface that shows two languages at once is `F5`'s | Display only, not persisted |
