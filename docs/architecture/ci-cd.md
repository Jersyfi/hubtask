...
| `contract` | Responses against `openapi.yaml`, events against JSON schemas, the closed sets a descriptor and the contract each declare, OpenAPI diff against the last tag | Compatibility |
...
| `data` | Migration check against the previous state, retention tests RE-1…RE-9, backup round trip BK-1 (local, S3, WebDAV, SFTP), sync tests SY-1…SY-12 | Data guarantees |
...
| `engines` | The built web application loaded in Chromium, Firefox and WebKit through Playwright, pinned ([ADR-0048](../adr/ADR-0048-browser-job-driver.md)), and ADR-0044's feature table asserted in each: a modal dialog traps focus and makes the page inert, the focus ring lands where rule 5 puts it, a hidden label is in the tree and off the screen, anchor positioning exists (`apps/webapp/e2e/`, `pnpm --filter @hubtask/webapp test:engines`). The one job that runs a browser, and what `support-matrix.md` §5's `supported` stands on | The browser row |
| `engine-session` | The sync engine's conformance run against the reference Compose stack (`make gate-engine-conformance`, `scripts/sync-engine-conformance.sh`): the first-party engine driven through its own API, each of `offline-sync.md` §9's eight client requirements answered by its number - the client's half of the hubctl session's `sync-conformance` section, in a stack of its own because that session spends the rate limiter's burst in its last sections. `1.0.0` prerequisite 17 | The client's half of the offline synchronisation |
...
| CI-3 | ~~Enforce image signature verification at deployment (Kyverno/Sigstore policy)~~ — built in P-15 (`0.9.0`), in the two places a deployment happens. The chart renders a Kyverno `ClusterPolicy` behind `imagePolicy.enabled` (default off — the kind needs Kyverno, and a `ClusterPolicy` is the cluster owner's to install): `verifyImages` for `ghcr.io/jersyfi/hubtask`, keyless, GitHub's OIDC issuer and the subjects the release workflow and the integration deploy sign under, `failurePolicy: Fail`; `make gate-chart` proves it renders with its identity, only when asked, and never without a subject, and `gate-selftest` proves that check bites. The integration deploy (`deploy.yml`) signs the per-commit image under its own identity and runs `cosign verify` against it before `helm upgrade`, so an unsigned or foreign image never reaches the one cluster this repository deploys to; `cosign-installer` is pinned as in `release.yml`. The platform that runs production decides whether to apply the policy (ADR-0046) | Closed (P-15) |
