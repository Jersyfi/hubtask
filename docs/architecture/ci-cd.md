...
| CI-5 | Where the component workbench is served from — answered with ADR-0038: `workbench.hubtask.eu`, the same IONOS webspace in its own directory, published by `.github/workflows/workbench.yml` on every change to `packages/design-system/`. What remains with the owner: the DNS record for the subdomain, the webspace mapping from it to a directory, and setting `WORKBENCH_REMOTE_DIR` — the host, the host key and the credentials are the website's. The workflow builds and skips politely until then, and refuses outright to mirror into the webspace root or the website's directory | Closed (ADR-0038), configuration pending |
