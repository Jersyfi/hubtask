...
| AI provider (kind, endpoint, models, jurisdiction, consent, sealed key) | `ai_provider` | `NON_PERSONAL` (+ `SECRET` for `api_key_enc`) | Where a workspace's content may be sent, and whether it may (J-02) | Consent; the transfer is the controller's assessment ([ai-providers.md](./ai-providers.md)) | The lifetime of the configuration; removed with the workspace | `CASCADE`. The key is sealed under E-02 and answered by no read; removing the provider removes it. **What is sent** under this configuration is item and jumble content, which is catalogued as itself above — this row is the configuration, not a second copy of the content | 
...
| Comments (what somebody wrote, and since migration 0084 what the server filed on their behalf: the displaced version of a free-text field that lost a merge, marked `SYSTEM` with a message code as its heading — [offline-sync.md](../architecture/offline-sync.md) §5) | `comment` | `PERSONAL_CONTENT` | Collaboration; the recovery of a merge's loser | As above | `CASCADE` / `ANONYMIZE` (the author) |
...
| Full-text index | `work_item.search_document` (and `work_item.search_vector` until a later migration drops it), optionally a vector index; `work_item.search_configuration` is the name of the text search configuration that built the document (M-09) — a configuration name, not content | `PERSONAL_CONTENT` (derived) | Search | With the source row | `CASCADE` |
...
| Calendar address | `work_item.calendar_uid` | `NON_PERSONAL` | The UID a calendar client chose for an entry it made, so that the CalDAV tree answers the entry at that address (P-07, issue #721); an opaque string, set once and never edited | With the source row | `CASCADE` |
...
| Who deleted it | `work_item.deleted_by_type`, `work_item.deleted_by_id`, and the same pair on `container` | `PERSONAL_BASIC` (the identifier; the kind is `NON_PERSONAL`) | The trash shows what was deleted, when, and by whom — F2-14's acceptance. The audit trail holds the same fact behind `AUDIT_READ`, which most members do not have | With the row, so 30 days in the trash | `CASCADE` (it goes when the row is hard-deleted) / `ANONYMIZE` (an erased account leaves the kind and its identifier resolves to the erasure's marker, as every other actor reference does) |
...
| The clock each field was last written under, for offline merging (entity, field, reading with the device's identifier) | `field_clock` | `PERSONAL_TECHNICAL` (a device identifier inside the reading) | Deciding a push's field against the server's copy ([offline-sync.md](../architecture/offline-sync.md) §4.2) | With the entity | `CASCADE` (the purge removes them beside the entry) |
...
| The description a template is drafted from (who asked, the words) | `ai_request` | `PERSONAL_CONTENT` | Template generation (P-11): the one question whose material exists in no row the workspace already holds, so the job's payload can keep carrying references only | Minutes — the job that reads it deletes it when it ends; one an abandoned job left behind goes with the suggestions' thirty-day sweep | `RETENTION` / `CASCADE` |
...
| Instance events (the act, the tenant's identifier and slug, the operator's label, counts — never content) | `instance_event` | `PERSONAL_TECHNICAL` (the operator's own label; no end-user data) | Evidence of control-plane acts whose per-tenant trail cannot hold them — above all a tenant's hard delete, after which that tenant's audit chain is gone by design (H-06, audit.md §6) | Permanent (it is the proof the deletion happened) | `IMMUTABLE` (append-only; the operator's label is the installation's own administrator, removable only by the operator at the database) |
...
| Audit anchors (chain hash, the backup target's identifier as destination, the object's digest as receipt) | `audit_anchor` | `NON_PERSONAL` | Proof that the audit chain was not rewritten (ADR-0017); written daily since P-13 and read back by `:verify` | With the workspace — the row is append-only for the application role and dies with the tenant | `IMMUTABLE` / `CASCADE` |
