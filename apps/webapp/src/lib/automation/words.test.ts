...
import { generatedName, grouped, kindWord, sentence, type Catalogue, type Names } from './words.ts';
...
test('a kind has a word in the catalogue or is read as words', () => {
...
test('the sentence and the name are built from the rule, and follow it', () => {
  ...
  assert.match(said, /conditions=app\.flow\.sentence_only_when\(conditions=app\.flow\.subject_type app\.flow\.op_is TASKapp\.flow\.sentence_andapp\.flow\.sentence_expression\)/);
  assert.match(said, /steps=app\.flow\.action\.add_labelapp\.flow\.sentence_then_sepapp\.flow\.sentence_branch\(condition=app\.flow\.subject_due app\.flow\.op_has,then=send webhook,else=app\.flow\.sentence_nothing\)/);
  draft.actions = [];
