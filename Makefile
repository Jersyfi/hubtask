...
# written against are the package's own build (`pnpm --filter @hubtask/sdk-typescript build`).
.PHONY: sdk
sdk:
	$(GO) run ./tools/sdkgen api/openapi.yaml sdk/typescript/src/client.gen.ts sdk/python/hubtask
...
## gate-sdk: The generated Python SDK parses
# `make generate` writes it and `gate-quick` catches a drift, so what the contract produces is
# already checked. What was not checked is whether it is Python at all: the only thing that ever
...
#
# python3 is required rather than skipped. It is already a dependency of the Compose gate
# (scripts/compose-smoke.sh reads JSON with it) and it is on every runner and every developer
# machine this project supports - and a gate that quietly does nothing is worse than no gate.
.PHONY: gate-sdk
gate-sdk:
	@command -v python3 >/dev/null || { \
		echo "python3 is missing - it is what proves the generated Python SDK parses"; exit 1; }
	@cache="$$(mktemp -d)"; \
		PYTHONPYCACHEPREFIX="$$cache" python3 -m compileall -q sdk/python > /tmp/hubtask-sdk-compile.$$$$ 2>&1; \
		status=$$?; \
		rm -rf "$$cache"; \
		if [ "$$status" -ne 0 ]; then \
			echo "the generated Python SDK does not parse:"; \
			cat /tmp/hubtask-sdk-compile.$$$$; rm -f /tmp/hubtask-sdk-compile.$$$$; \
			echo "it is generated by 'make sdk' from api/openapi.yaml - tools/sdkgen is where to fix it"; \
			exit 1; \
		fi; \
		rm -f /tmp/hubtask-sdk-compile.$$$$
